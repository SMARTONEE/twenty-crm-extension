...
export default defineContentScript({
  ...
  main(ctx) {
    ...
    // Field editor state — dynamic from user preferences
    let showFieldEditor = false;
    let editingField: string | null = null;
    let availableFields: Array<{ name: string; label: string; type: string; options?: Array<{ value: string; label: string }> }> = [];
    let fieldValues: Record<string, Array<{ value: string; label: string }>> = {};
    ...
    function render() {
      ...
      if (showFieldEditor && editingField) {
        ...
        const values = fieldValues[editingField] || [];
